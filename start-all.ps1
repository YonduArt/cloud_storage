...
if (-not (Test-AiWorker)) {
    throw "AI worker did not start on http://localhost:8090. Check the PowerShell window opened for ai-worker."
}

docker compose -f docker-compose.yml -f docker-compose.local-ai.yml up --build -d postgres backend frontend
